...
const validator = read("references/workflows/protokoll-validierung.md");
...
expect(
  market.plugins[0].source?.source === "local" && market.plugins[0].source?.path === "./plugins/eba-protokoll",
  "marketplace entry points at the repo-local Codex plugin path",
);

// Native Codex packaging contract. Claude command/agent directories are not
// plugin components; their behavior must live in skills and references.
expect(pluginManifest.name === "eba-protokoll", "plugin manifest uses the native Codex name");
expect(!existsSync(join(repoRoot, "commands")), "plugin has no unsupported commands directory");
expect(!existsSync(join(repoRoot, "agents")), "plugin has no legacy root agents directory");
expect(!existsSync(join(repoRoot, ".claude-plugin")), "plugin has no Claude manifest directory");
...
const expectedSkills = [
  ...
  "protokoll-lp1-4",
  "protokoll-lp5",
  "protokoll-themen-extrahieren",
  "protokoll-validieren",
  "teilnehmer-aufloesen",
  "transkript-vorbereiten",
];
for (const skillName of expectedSkills) {
  const skillPath = `skills/${skillName}/SKILL.md`;
  const uiPath = `skills/${skillName}/agents/openai.yaml`;
  expectFile(skillPath);
  expectFile(uiPath);
  const skill = read(skillPath);
  const ui = read(uiPath);
  expect(
    new RegExp(`^name: ${skillName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "m").test(skill),
    `${skillPath} name matches its directory`,
  );
  expect(ui.includes(`$${skillName}`), `${uiPath} default prompt mentions $${skillName}`);

  for (const match of skill.matchAll(/`(\.\.\/\.\.\/(?:references|scripts)\/[^`]+)`/g)) {
    const target = join(repoRoot, "skills", skillName, match[1]);
    expect(existsSync(target), `${skillPath} resource exists: ${match[1]}`);
  }
}

const forbiddenClaudeTokens = [
  "allowed-tools:",
  "argument-hint:",
  "$ARGUMENTS",
  ".claude-plugin",
...
for (const relativePath of textFiles(".")) {
  ...
  for (const token of forbiddenClaudeTokens) {
    expect(!content.includes(token), `${relativePath} contains no Claude-only token ${token}`);
  }
}
