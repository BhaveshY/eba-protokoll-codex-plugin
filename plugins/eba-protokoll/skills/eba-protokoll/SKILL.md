...
description: >-
  ...
  then follows the matching native Codex format skill.
---

# EBA-Protokoll: Auto-Erkennung und Routing

Verwende diese Skill als **Haupteinstieg**, wenn ein Transkript ohne explizite
Vorlage vorliegt. Erkenne das passende Format und folge der spezifischen
Format-Skill.

## Plugin-Ressourcen auflösen

Bestimme zuerst das Verzeichnis dieser `SKILL.md`. Setze `<plugin-root>` auf
`../..` relativ zu diesem Verzeichnis. Löse alle Pfade in dieser Skill relativ
zur Skill-Datei auf, niemals relativ zum Arbeitsverzeichnis des Nutzers.
...
Lies die ganze Datei. Folgende Metadaten heuristisch ableiten:
...
### 4. Format-Skill anwenden

- **Gesprächsnotiz** → `<plugin-root>/skills/gespraechsnotiz/SKILL.md`.
- **Protokoll-einfach** → `<plugin-root>/skills/protokoll-einfach/SKILL.md`.
- **LP1-4** (inkl. BIM) → `<plugin-root>/skills/protokoll-lp1-4/SKILL.md`.
- **LP5** → `<plugin-root>/skills/protokoll-lp5/SKILL.md`.

Lies die gewählte Skill-Datei vollständig und folge ihren Anweisungen. Bei
komplexen Transkripten nutze zusätzlich `protokoll-themen-extrahieren`; bei
generischen Sprecherlabels `teilnehmer-aufloesen`.
...
Die Format-Skills führen das Rendering mit
`<plugin-root>/scripts/render_protokoll.py` aus. Die vollständige Pipeline
ist beschrieben in
...
- `../../references/workflows/teilnehmer-aufloesung.md`
- `../../references/workflows/protokoll-validierung.md`
